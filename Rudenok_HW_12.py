...
# Клас для створення датасету собак з CIFAR-10
class DogDataset(Dataset):
    def __init__(self, cifar_dataset, max_samples=None):
        ...
        print("Створення датасету собак...")

        for i, (img, label) in enumerate(cifar_dataset):
            if max_samples and sample_count >= max_samples:
                break

            # Створюємо бінарну класифікацію: 1 - собака, 0 - не собака
            if label == 5 and dog_count < (max_samples // 2 if max_samples else 2500):  # Собака (обмежуємо кількість)
                self.data.append(img)
                self.labels.append(1)
                dog_count += 1
                sample_count += 1
            elif label != 3 and i % 5 == 0 and non_dog_count < (max_samples // 2 if max_samples else 2500):  # Не собака
                self.data.append(img)
                self.labels.append(0)
                non_dog_count += 1
                sample_count += 1

        print(f"Створено датасет: {dog_count} собак, {non_dog_count} не-собак")
...
class OptimizedDogDetectionCNN(nn.Module):
    def __init__(self, dropout_rate=0.3):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(dropout_rate),
            nn.Linear(64, 2)  # 2 класи: собака/не собака
        )
...
# Функція для відображення передбачень на тестовому наборі
def show_test_predictions(model, test_loader, device, class_names=["Не собака", "Собака"], num_images=8):
    """
    Відображає кілька прикладів із тестового набору з передбаченнями моделі.
    """
    model.eval()
    images_shown = 0
    ...
    with torch.no_grad():
        for images, labels in test_loader:
            ...
            for i in range(images.size(0)):
                ...
                img = (img - img.min()) / (img.max() - img.min())  # нормалізація для відображення
                ...
                # колір підпису залежить від правильності
                color = "green" if preds[i] == labels[i] else "red"
                plt.title(f"{pred_label} ({confidence:.1f}%)\nІстина: {true_label}", fontsize=10, color=color)
...
def main():
    ...
    try:
        ...
        # Створення датасетів собак (з обмеженням для швидкого тестування)
        train_dataset = DogDataset(cifar_train, max_samples=5000)  # Зменшуємо для швидкості
        test_dataset = DogDataset(cifar_test, max_samples=1000)
        ...
        if train_losses and train_accs and test_accs:
            ...
            # Створюємо модель з тією ж конфігурацією, що й під час тренування
            best_model = OptimizedDogDetectionCNN(dropout_rate=0.2).to(device)

            # Завантажуємо збережені ваги
            checkpoint = torch.load("best_dog_detection_model.pth", map_location=device)
            best_model.load_state_dict(checkpoint['model_state_dict'])

            # Переключаємо модель у режим оцінки
            best_model.eval()

            print(f"Завантажено найкращу модель з точністю: {best_acc:.2f}%")

            # Візуалізація передбачень з тесту
            # Передаємо найкращу модель у show_test_predictions
            show_test_predictions(best_model, test_loader, device, num_images=8)
